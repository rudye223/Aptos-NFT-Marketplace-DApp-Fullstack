...
const ChatPage: React.FC = () => {
    ...
    useEffect(() => {
         if (location.state?.initialChat) {
           setInitialChat(location.state.initialChat)
          setRecipientAddress(location.state.recipient)

        };
         if (account) {
           checkUserInitialization();
            if(location.state?.recipient){
               checkExistingChat(account.address, location.state.recipient)
            }
        }
    }, [account, location]);
   ...
   const startPolling = () => {
    ...
    const checkExistingChat = async (userAddress: string, recipientAddress: string) => {
         try {
              const response = await client.view({
                ...
                console.log("alreadychat::", response)
              if(response[0]){
...
// length
// : 
// 1. so this is the response the getchatid returns from the smart contract. im pasting this to give u context so we can handle the data returned properly.  as u can see it return a vec or something with the chat id in it. so after we fetch the chats we look for a chat with that chatidand automaatically select it